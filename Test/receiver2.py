...
class MyGame(arcade.Window):
    ...
    def setup_rabbitmq(self):
        ...
        # Declare reply queue (needed for basic_publish from consuming thread)
        try:
            ...
            print(f"Error connecting to RabbitMQ while declaring reply queue: {e}")
        ...
        try:
            ...
            def callback(ch, method, properties, body):
                ...
                try:
                    ...
                    # Move the ball and get position BEFORE sending reply
                    self.process_messages(0) # delta_time is not used here

                    # Send ball position back to sender
                    ball_position = {"x": self.ball_x, "y": self.ball_y}
                    channel.basic_publish(
                        exchange='',
                        routing_key=REPLY_QUEUE_NAME,  # Отправляем в очередь для ответов
                        body=json.dumps(ball_position)
                    )
                    print(f" [x] Sent ball position: {ball_position} to reply queue")
